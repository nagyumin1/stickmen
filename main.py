...
# 3. HTML & JavaScript 코드 (싱글 따옴표 단일화 및 이모지 제거로 에러 원천 봉쇄)
stickman_html = (
    ...
    '            s.vy = -12; s.state = "surprised"; s.stateTimer = 45;'
    '        }'
    '    });'
    '});'
    'class Stickman {'
    ...
    '        this.state = "walk"; this.stateTimer = 0; this.groundY = canvas.height - 30;'
    '    }'
    '    update() {'
    ...
    '            ctx.font = "bold 16px sans-serif"; ctx.fillText("!!", this.x - 8, this.y - this.size * 2.5);'
    '        } else if (this.state === "annoyed") {'
    '            ctx.font = "bold 16px sans-serif"; ctx.fillText("X!", this.x - 8, this.y - this.size * 2.5);'
    '        }'
    ...
    '        if (this.y < this.groundY) {'
    '            leftLegAngle = 0.4; rightLegAngle = -0.4; leftArmAngle = -0.8; rightArmAngle = 0.8;'
    '        } else if (this.state === "surprised") {'
    '            leftLegAngle = -0.2; rightLegAngle = 0.2; leftArmAngle = -2.2; rightArmAngle = 2.2;'
    '        } else {'
    '            let swing = Math.sin(this.walkCycle) * 0.6;'
    '            leftLegAngle = swing; rightLegAngle = -swing; leftArmAngle = -swing * 0.5; rightArmAngle = swing * 0.5;'
    '        }'
    '        let legLength = this.size * 1.8;'
    '        ctx.beginPath();'
    '        ctx.moveTo(this.x, torsoBottom); ctx.lineTo(this.x + Math.sin(leftLegAngle) * legLength, torsoBottom + Math.cos(leftLegAngle) * legLength);'
    '        ctx.moveTo(this.x, torsoBottom); ctx.lineTo(this.x + Math.sin(rightLegAngle) * legLength, torsoBottom + Math.cos(rightLegAngle) * legLength);'
    '        ctx.stroke();'
    '        let armLength = this.size * 1.5; let armY = torsoTop + this.size * 0.5;'
    '        ctx.beginPath();'
    '        ctx.moveTo(this.x, armY); ctx.lineTo(this.x + Math.sin(leftArmAngle) * armLength, armY + Math.cos(leftArmAngle) * armLength);'
    '        ctx.moveTo(this.x, armY); ctx.lineTo(this.x + Math.sin(rightArmAngle) * armLength, armY + Math.cos(rightArmAngle) * armLength);'
    '        ctx.stroke();'
    '    }'
    '}'
    'const stickmen = ['
    '    new Stickman(200, 100, "#FF3333"),'
    '    new Stickman(400, 100, "#33FF33"),'
    '    new Stickman(600, 100, "#3333FF"),'
    '    new Stickman(800, 100, "#FFFF33")'
    '];'
    'function handleStickmanCollisions() {'
    '    for (let i = 0; i < stickmen.length; i++) {'
    '        for (let j = i + 1; j < stickmen.length; j++) {'
    '            let s1 = stickmen[i]; let s2 = stickmen[j];'
    '            let dx = s2.x - s1.x; let dy = s
